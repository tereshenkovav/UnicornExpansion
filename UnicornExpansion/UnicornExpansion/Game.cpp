...
int Game::getMaxEnergy() const
{
	int max = 0;
	for (int i = 0; i < units.size(); i++)
		if (units[i].isComponent<ComponentStore>()) max += units[i].getComponent<ComponentStore>()->getCapacity();
	return max;
}
...
{
	return std::format("{}/{}", getEnergy(), getMaxEnergy());
}
	...
	for (auto & unit: units)
		if (unit.isKilled()) {
			if (unit.isComponent<ComponentUnicorn>()) {
				...
				if (unit.isComponent<ComponentResource>()) addGameEvent(AudioEffect::CrystallKilled, unit.getView());
			}
		}
