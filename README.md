...
как наборы заголовочных файлов и собранные .lib-файлов 64-битной версии для Visual Studio 2019.

### Сборка для MinGW

Используется проект CMake и готовые бинарные файлы для SFML 3.0, согласно
рекомендациям разработчиков SFML, нужно использовать конкретную версию MinGW -
[14.2](https://github.com/brechtsanders/winlibs_mingw/releases/download/14.2.0posix-19.1.1-12.0.0-ucrt-r2/winlibs-x86_64-posix-seh-gcc-14.2.0-mingw-w64ucrt-12.0.0-r2.7z)
Все зависимости аналогично включены в проект в виде
наборов заголовочных файлов и собранных .a-файлов 64-битной версии для конкретной версии MinGW.
Для сборки вызывается файл `build_mingw64.bat` из корня репозитория,
готовый бинарный файл будет размещен в каталоге bin, для запуска нужно поместить в каталог с exe-файлом
библиотеки dll от SFML 3.0 для MinGW и запускать его командой `UnicornExpansion.exe ..\data`
